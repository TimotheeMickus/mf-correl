...
def get_non_systematic_definitions_annots():
	...
	data = zip(data[0], map(read_file, data[1]))
	...
	with open(annot_file, "w") as ostr:
		for filename, filecontents in data:
			for j_dict in filecontents:
				...
				print(filename, sent_a, sent_b, input(msg), file=ostr, sep="\t", flush=True)
	
def get_synonym_based_definitions_annots():
	...
	with open(annot_file, "w") as ostr:
		for filename, filecontents in data:
			for j_dict in filecontents:
				...
				print(filename, sent_a, sent_b, *annot, file=ostr, sep="\t", flush=True)


	

def get_similar_prefix_definitions_annots():
	base_dir = "rnk/filtered_nonnorm"
	raw_datafile = "dataset/datasets/tsv/unique-definiendum/dataset-rd-choice.01.tsv"
...
def merge_annots():
	...
	with open(annots_prefix) as istr:
		...
		data_prefix = [[*l, #data_rare[k1], data_rare[k2], 
				data_synonyms[k1],  data_synonyms[k2], *data_rank[tuple(l[:-1])]
			] for l,k1,k2 in data_prefix]

	manual_annots_file = "manual_annots.filtered_nonnorm.tsv"
	with open(manual_annots_file, "w") as ostr:
...
if __name__ == "__main__":
	#get_rare_sense_annots()
	#get_non_systematic_definitions_annots() # this one will not be ran due to the limited interest it brings
	get_synonym_based_definitions_annots()
	get_similar_prefix_definitions_annots()
	get_score_and_rank_annots()
	merge_annots()

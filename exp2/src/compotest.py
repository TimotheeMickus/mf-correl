...
def sample_pairs(sentences, restrict_dataset_size=None, sample_size=1024, prevout2="../pairs-fn", prevout1="okays"):
	...
	if False and prevout1 and prevout2:
		...
		with open(prevout2) as istr:
			...
			lines = set(lines)
	sample = sample or range(len(sentences))
...
def correlation_fn(single_arg):
	...
	"""
	sample, sentences, tree_idx, pos_decored_idx, meanings_idx, filtered_levenshtein, filtered_levenshtein_pos, w2c, remap, dump_vals, embeddings, average_distance, min_df = single_arg
	...
	single_args = ([sentences, tree_idx, pos_decored_idx, meanings_idx, filtered_levenshtein, filtered_levenshtein_pos, w2c, remap, dump_vals, embeddings, average_distance, min_df] + list(s) for s in sample)
	for v in tqdm.tqdm(pool.imap_unordered(compute_val, single_args, 200), total=len(sample), file=sys.stderr, leave=False):
		pass
	pool.close()
	pool.join()
	"""if dump_vals:
...
if __name__ == "__main__":
	...
	p.add_argument('--ft_embs', default="../cc.en.300.bin", type=str)
	p.add_argument('--annot_files', default=None, type=str)
	p.add_argument('--baseline_support', default=30, type=int)
	p.add_argument('--average_distance', default=1.563646, type=float)
	...
	_ = correlation_fn([sample, sentences, tree_idx, pos_decored_idx, meanings_idx, filtered_levenshtein, filtered_levenshtein_pos, w2c, False, 'scores.json',  embeddings, average_distance, min_df])
